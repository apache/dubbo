...
public class MetricsInitEvent extends TimeCounterEvent {
    ...
    public MetricsInitEvent(ApplicationModel source, TypeWrapper typeWrapper) {
        super(source, typeWrapper, TimePair.start());
    }

    public static MetricsInitEvent toMetricsInitEvent(InitServiceMethodEvent event) {
        ApplicationModel applicationModel = event.getApplicationModel();
        Invocation invocation = event.getInvocation();
        boolean serviceLevel = MethodMetric.isServiceLevel(event.getApplicationModel());
