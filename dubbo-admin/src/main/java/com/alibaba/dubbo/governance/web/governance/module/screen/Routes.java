...
/**
 * Providers. URI: /services/$service/routes
 * 
 * @author ding.lid
...
public class Routes extends Restful {
	...
	static String[][] when_names = { { "method", "method", "unmethod" },
	...
	public void index(Map<String, Object> context) {
		...
		List<Route> routes;
		if (service != null && service.length() > 0 && address != null && address.length() > 0) {
	...
	public void show(Map<String, Object> context) {
		try {
			...
			Map<String, RouteRule.MatchPair>[] paramArray = new Map[] { routeRule.getWhenCondition(),
					routeRule.getThenCondition() };
			...
			context.put("methods", CollectionUtils.sort(new ArrayList<String>(providerService
					.findMethodsByService(route.getService()))));
	...
	public void add(Map<String, Object> context) {
		...
		if (service != null && service.length() > 0 && !service.contains("*")) {
			...
			context.put("methods",
					CollectionUtils.sort(new ArrayList<String>(providerService.findMethodsByService(service))));
		} else {
	...
	public boolean create(Map<String, Object> context) {
		...
		if (StringUtils.isNotEmpty(service) && StringUtils.isNotEmpty(name)) {
			...
			RouteRule routeRule = RouteRule.createFromNameAndValueListString(when_name2valueList,
					notWhen_name2valueList, then_name2valueList, notThen_name2valueList);
	...
	public boolean update(Map<String, Object> context) {
		...
		if (idStr != null && idStr.length() > 0) {
			...
			if (StringUtils.isNotEmpty((String) context.get("name"))) {
				...
				// RelateUserUtils.addOwnersOfService(usernames,
				// route.getService(), ownerDAO);
	...
	public void preview(Map<String, Object> context) throws Exception {
		...
		if (StringUtils.isNotEmpty(consumerid)) {
			...
			Map<String, String> result = RouteUtils.previewRoute(consumer.getService(), consumer.getAddress(),
					consumer.getParameters(), serviceUrls, route, null, null);
			context.put("route", route);
			...
			Map<String, String> result = RouteUtils
					.previewRoute(service, address, null, serviceUrls, route, null, null);
			context.put("route", route);
