...
public class Tool {
	...
	public boolean checkUrl(User user, String uri) {
		return true;
		/*
		 * if(!User.ROOT.equals(user.getRole())){ List<String> disabledSysinfo =
		 * new ArrayList<String>(); List<String> disabledSysmanage = new
		 * ArrayList<String>(); Map<String, Boolean> features =
		 * daoCache.getFeatures(); if (features.size() > 0){
		 * for(Entry<String,Boolean> feature : features.entrySet()){
		 * if(feature.getKey().startsWith("Sysinfo") && !feature.getValue()){
		 * disabledSysinfo.add(feature.getKey().replace(".",
		 * "/").toLowerCase()); }else
		 * if(feature.getKey().startsWith("Sysmanage") && !feature.getValue()){
		 * disabledSysmanage.add(feature.getKey().replace(".",
		 * "/").toLowerCase()); } } if(uri.startsWith("/sysinfo")){ for(String
		 * disabled : disabledSysinfo){ if (uri.contains(disabled)){ return
		 * false; } } } if(uri.startsWith("/sysmanage")){ for(String disabled :
		 * disabledSysmanage){ if (uri.contains(disabled)){ return false; } } }
