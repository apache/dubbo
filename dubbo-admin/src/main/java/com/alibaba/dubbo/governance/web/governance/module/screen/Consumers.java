...
public class Consumers extends Restful {
	...
	public void index(Map<String, Object> context) throws Exception {
		...
		if (consumers != null && consumers.size() > 0) {
			for (Consumer consumer : consumers) {
				...
				consumer.setProviders(RouteUtils.route(consumer.getService(), consumer.getAddress(),
						consumer.getParameters(), providers, overrides, routes, null, routed));
	...
	public void show(Long id, Map<String, Object> context) {
		...
		List<Route> routed = new ArrayList<Route>();
		consumer.setProviders(RouteUtils.route(consumer.getService(), consumer.getAddress(), consumer.getParameters(),
				providers, overrides, routes, null, routed));
		consumer.setRoutes(routed);
		OverrideUtils.setConsumerOverrides(consumer, overrides);
	...
	public boolean update(Consumer newConsumer, Map<String, Object> context) {
		...
		List<Override> overrides = overrideService
				.findByServiceAndAddress(consumer.getService(), consumer.getAddress());
		OverrideUtils.setConsumerOverrides(consumer, overrides);
