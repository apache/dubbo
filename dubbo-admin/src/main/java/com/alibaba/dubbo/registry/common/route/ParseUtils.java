...
public class ParseUtils {
	 ...
	 * 
	 * @param expression
	 *            含有变量的表达式字符串。表达式中的变量名也可以用<code>{}</code>括起来。
	 * @param params
	 *            变量集。变量名可以包含<code>.</code>、<code>_</code>字符。
	 * @return 完成interpolation后的字符串。 如：
	 *         <code><pre>xxx${name}zzz -> xxxjerryzzz</pre></code>
	 *         （其中变量name="jerry"）
	 * @throws IllegalStateException
	 *             表达式字符串中使用到的变量 在变量集中没有
	 */
	 ...
	 *            进行Glob模式的值
	 */
	public static boolean isMatchGlobPatternsNeedInterpolate(Collection<String> patternsNeedInterpolate,
	...
	public static boolean hasIntersection(String glob1, String glob2) {
		...
		if (glob1.contains("*") && glob2.contains("*")) {
			...
			if (!s11.startsWith(s21) && !s21.startsWith(s11))
				return false;
			if (!s12.endsWith(s22) && !s22.endsWith(s12))
				return false;
			return true;
	 ...
	 *            在输出的Map的Key加上统一前缀。
	 * @param query
	 *            Query String，形如：<code>key1=value1&key2=value2</code>
	 * @return Query String为<code>key1=value1&key2=value2</code>，前缀为
	 *         <code>pre.</code>时， 则返回
	 *         <code>Map{pre.key1=value1, pre.key=value2}</code>。
	 */
	...
	public static Map<String, String> parseQuery(String keyPrefix, String query) {
		...
		while (matcher.find()) { // 逐个匹配
			...
			if (separator == null || separator.length() == 0 || "&".equals(separator)) {
				if (key != null)
					throw new IllegalStateException("Illegal query string \"" + query + "\", The error char '"
							+ separator + "' at index " + matcher.start() + " before \"" + content + "\".");
				key = content;
		 ...
		 * IllegalStateException("Illegal route rule \"" + query +
