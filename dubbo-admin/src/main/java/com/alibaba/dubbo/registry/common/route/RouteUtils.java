...
public class RouteUtils {
	...
	public static boolean matchRoute(String consumerAddress, String consumerQueryUrl, Route route,
			Map<String, List<String>> clusters) {
		RouteRule rule = RouteRule.parseQuitely(route);
		Map<String, RouteRule.MatchPair> when = RouteRuleUtils.expandCondition(rule.getWhenCondition(),
				"consumer.cluster", "consumer.host", clusters);
	...
	public static Map<String, String> previewRoute(String serviceName, String consumerAddress, String queryUrl,
			Map<String, String> serviceUrls, Route route, Map<String, List<String>> clusters, List<Route> routed) {
		if (null == route) {
	...
	public static List<Provider> route(String serviceName, String consumerAddress, String consumerQueryUrl,
			List<Provider> providers, List<Override> overrides, List<Route> routes, Map<String, List<String>> clusters,
			List<Route> routed) {
		if (providers == null) {
			return null;
		}
		Map<String, String> urls = new HashMap<String, String>();
		urls.put("consumer://" + consumerAddress + "/" + serviceName, consumerQueryUrl); // not
																							// empty
																							// dummy
																							// data
	...
	/**
	 * @param serviceName
	 *            服务名，如{@code com.alibaba.morgan.MemberService}
	 * @param consumerAddress
	 *            消费者地址，如{@code 192.168.1.3:54333}
	 * @param consumerQueryUrl
	 *            订阅参数，如
	 *            <code>aplication=nasdaq&dubbo=2.0.3&methods=updateItems,validateNew&revision=1.7.0</code>
	 * @param serviceUrls
	 *            提供者
	 * @param routes
	 *            全部路由
	...
	public static Map<String, String> route(String serviceName, String consumerAddress, String consumerQueryUrl,
			Map<String, String> serviceUrls, List<Route> routes, Map<String, List<String>> clusters, List<Route> routed) {
		...
		String[] methods = methodsString == null || methodsString.length() == 0 ? new String[] { Route.ALL_METHOD }
				: methodsString.split(ParseUtils.METHOD_SPLIT);
		for (String method : methods) {
			consumerSample.put("method", method);
			// NOTE:
			// <*方法>只配置 <no method key>
			// method1方法匹配 <no method key> 和 <method = method1>, 此时要把<no method
			// key>的Route的优先级降低即可
			if (routes != null && routes.size() > 0) {
				for (Route route : routes) {
					if (isSerivceNameMatched(route.getService(), serviceName)) {
						...
						// 当满足when条件时
						if (rule != null
								&& RouteRuleUtils.isMatchCondition(rule.getWhenCondition(), consumerSample,
										consumerSample)) {
							...
							if (then != null) {
								Map<String, Map<String, String>> tmp = getUrlsMatchedCondition(then, consumerSample,
										url2ProviderSample);
	...
	static Map<Long, RouteRule> route2RouteRule(List<Route> routes, Map<String, List<String>> clusters) {
		...
		if (clusters != null && clusters.size() > 0) {
			...
			for (Map.Entry<Long, RouteRule> entry : rules.entrySet()) {
				...
				Map<String, RouteRule.MatchPair> when = RouteRuleUtils.expandCondition(rr.getWhenCondition(),
						"consumer.cluster", "consumer.host", clusters);
				Map<String, RouteRule.MatchPair> then = RouteRuleUtils.expandCondition(rr.getThenCondition(),
						"provider.cluster", "provider.host", clusters);
	...
	static Route getFirstRouteMatchedWhenConditionOfRule(String serviceName, Map<String, String> consumerSample,
			List<Route> routes, Map<Long, RouteRule> routeRuleMap) {
		...
		if (routes != null && routes.size() > 0) {
			for (Route route : routes) {
				if (isSerivceNameMatched(route.getService(), serviceName)) {
					...
					// 当满足when条件时
					if (rule != null
							&& RouteRuleUtils.isMatchCondition(rule.getWhenCondition(), consumerSample, consumerSample)) {
	...
	static boolean isSerivceNameMatched(String servicePattern, String serviceName) {
		...
		if (pip != -1) { // pattern有group
			if (pi == -1)
				return false; // servicename无group
