...
public class ChartsPageHandler implements PageHandler {
	...
	public Page handle(URL url) {
		...
		return new Page(nav.toString(), "Charts (" + rows.size() + ")", new String[] { "Method",
				"Requests per second (QPS)", "Average response time (ms)" }, rows);
	}
	...
	private List<String> toRow(File dir, String uri) {
		...
		if (new File(dir, MonitorService.SUCCESS + ".png").exists()) {
			...
			row.add("<a href=\"" + url + "\" target=\"_blank\"><img src=\"" + url
