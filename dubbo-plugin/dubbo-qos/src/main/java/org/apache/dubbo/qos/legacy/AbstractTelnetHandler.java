...
abstract class AbstractTelnetHandler implements TelnetHandler {

    protected ProviderModel findProvider(String service) {
    ...
    protected Invoker<?> findInvoker(ProviderModel provider) {
        for (Exporter<?> exporter : AbstractProtocol.getAllExporters()) {
            Invoker<?> invoker = exporter.getInvoker();
            if (provider.getServiceName().equals(invoker.getUrl().getPathKey())) {
                return invoker;
            }
        }
        return null;
