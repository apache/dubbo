...
@Cmd(name = "OfflineMetaData", summary = "offline service metadata and service instance", example = {
    "offline-metadata",
    "offline-metadata 5"
})
public class OfflineMetadata implements BaseCommand {

    private static final Logger logger = LoggerFactory.getLogger(OfflineMetadata.class);
    private final FrameworkModel frameworkModel;

    public OfflineMetadata(FrameworkModel frameworkModel) {
        this.frameworkModel = frameworkModel;
    }

    @Override
    public String execute(CommandContext commandContext, String[] args) {
        logger.info("received offlineMetaData command.");
        ...
        for (ApplicationModel applicationModel : applicationModels) {
            if (ArrayUtils.isEmpty(args)) {
                ...
                stringBuilder.append("offline metadata succeeded. App:").append(applicationModel.getApplicationName())
                    .append("\n");
            } else {
                try {
                    ...
                    frameworkExecutorRepository.nextScheduledExecutor()
                        .schedule(() -> ServiceInstanceMetadataUtils.refreshMetadataAndInstance(applicationModel),
