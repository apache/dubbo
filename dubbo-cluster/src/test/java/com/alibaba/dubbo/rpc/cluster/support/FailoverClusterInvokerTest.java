...
public class FailoverClusterInvokerTest {
	...
	public void testInvokeWithRuntimeException() {
		...
		FailoverClusterInvoker<FailoverClusterInvokerTest> invoker = new FailoverClusterInvoker<FailoverClusterInvokerTest>(
				dic);
		try {
	...
	public void testInvoke_retryTimes() {
		...
		EasyMock.reset(invoker1);
		EasyMock.expect(invoker1.invoke(invocation)).andThrow(new RpcException(RpcException.TIMEOUT_EXCEPTION))
				.anyTimes();
	...
	public void testInvokerDestoryAndReList() {
		final URL url = URL.valueOf("test://localhost/" + Demo.class.getName() + "?loadbalance=roundrobin&retries="
				+ retries);
