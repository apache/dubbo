...
import org.apache.dubbo.rpc.cluster.configurator.parser.mesh.rule.virtualservice.VirtualServiceSpec;
import org.apache.dubbo.rpc.cluster.configurator.parser.mesh.rule.virtualservice.destination.DubboDestination;
import org.apache.dubbo.rpc.cluster.configurator.parser.mesh.rule.virtualservice.destination.DubboRouteDestination;
        ...
        if (CollectionUtils.isNotEmpty(invokers)) {
            for (Invoker<T> invoker : invokers) {
                ...
                if (StringUtils.isNotEmpty(applicationName)
                        && !MeshRuleConstants.INVALID_APP_NAME.equals(applicationName)) {
    ...
    public synchronized void onRuleChange(String appName, List<Map<String, Object>> rules) {
        ...
        try {
            ...
            for (Map<String, Object> rule : rules) {
                if (MeshRuleConstants.DESTINATION_RULE_KEY.equals(rule.get(MeshRuleConstants.KIND_KEY))) {
                ...
                } else if (MeshRuleConstants.VIRTUAL_SERVICE_KEY.equals(rule.get(MeshRuleConstants.KIND_KEY))) {
