...
public class AppScriptStateRouter<T> extends AbstractStateRouter<T> implements ConfigurationListener {
    ...
    protected BitList<Invoker<T>> doRoute(
        ...
        logger.info(
                "[SCRIPT-ROUTER] AppScriptStateRouter.doRoute called with {} invokers [Thread: {}]",
                invokers.size(),
        ...
        for (int i = 0; i < invokers.size(); i++) {
            ...
            logger.info(
                    "[SCRIPT-ROUTER] Input invoker[{}]: {} (clusterID: {}) [Thread: {}]",
                    i,
                    address,
                    clusterID,
                    Thread.currentThread().getName());
        ...
        if (scriptRouter == null || !scriptRule.isValid() || !scriptRule.isEnabled()) {
            logger.info(
                    "[SCRIPT-ROUTER] Script router disabled or invalid, returning all invokers [Thread: {}]",
                    Thread.currentThread().getName());
