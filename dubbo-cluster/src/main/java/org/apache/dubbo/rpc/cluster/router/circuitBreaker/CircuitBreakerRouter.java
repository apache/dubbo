 ...
 */
public class CircuitBreakerRouter<T> extends AbstractStateRouter<T> implements ConfigurationListener {
    public static final String NAME = "CIRCUIT_BREAKER_ROUTER";
    private static final ErrorTypeAwareLogger logger = LoggerFactory.getErrorTypeAwareLogger(CircuitBreakerRouter.class);
    private static final String RULE_SUFFIX = ".circuitBreaker-router";

    private volatile CircuitBreakerRule circuitBreakerRule;
    private String application;
    private volatile BitList<Invoker<T>> invokers = BitList.emptyList();

    public CircuitBreakerRouter(URL url) {
        super(url);
    }
    ...
    public synchronized void process(ConfigChangedEvent event) {
        ...
        try {
            if (event.getChangeType().equals(ConfigChangeType.DELETED)) {
                this.circuitBreakerRule = null;
            } else {
                CircuitBreakerRule rule = CircuitBreakerRuleParser.parse(event.getContent());
                rule.init(this);
                this.circuitBreakerRule = rule;
            }
        } catch (Exception e) {
    ...
    public BitList<Invoker<T>> doRoute(BitList<Invoker<T>> invokers, URL url, Invocation invocation, boolean needToPrintMessage, Holder<RouterSnapshotNode<T>> nodeHolder, Holder<String> messageHolder) throws RpcException {
        ...
        final CircuitBreakerRule circuitBreakerRuleCopy = circuitBreakerRule;
        if (circuitBreakerRuleCopy == null || !circuitBreakerRuleCopy.isValid() || !circuitBreakerRuleCopy.isEnabled()) {
            if (needToPrintMessage) {
        ...
        if (StringUtils.isNotEmpty(tag)) {
            Set<String> addresses = circuitBreakerRuleCopy.getCircuitBreakernameToAddresses().get(tag);
            // filter by dynamic tag group first
            if (addresses != null) { // null means tag not set
                result = filterInvoker(invokers, invoker -> addressMatches(invoker.getUrl(), addresses));
                // if result is not null OR it's null but force=true, return result directly
                if (CollectionUtils.isNotEmpty(result) || circuitBreakerRuleCopy.isForce()) {
                    if (needToPrintMessage) {
            ...
            // FAILOVER: return all Providers without any tags.
                ...
                BitList<Invoker<T>> tmp = filterInvoker(
                    invokers,
                    invoker -> addressNotMatches(invoker.getUrl(), circuitBreakerRuleCopy.getAddresses())
                );
                if (needToPrintMessage) {
            ...
            Set<String> addresses = circuitBreakerRuleCopy.getAddresses();
    ...
    public boolean isRuntime() {
        return circuitBreakerRule != null && circuitBreakerRule.isRuntime();
    }

    @Override
    public boolean isForce() {
        // FIXME
        return circuitBreakerRule != null && circuitBreakerRule.isForce();
    }
    ...
    public void notify(BitList<Invoker<T>> invokers) {
        ...
        synchronized (this) {
            if (!providerApplication.equals(application)) {
                ...
                if (this.circuitBreakerRule != null) {
                    CircuitBreakerRule newRule = CircuitBreakerRuleParser.parse(this.circuitBreakerRule.getRawRule());
                    newRule.init(this);
                    this.circuitBreakerRule = newRule;
                }
            }
        }
    }
    ...
    // for testing purpose
    public void setCircuitBreakerRule(CircuitBreakerRule tagRouterRule) {
        this.circuitBreakerRule = tagRouterRule;
    }
}
