...
public class TagStateRouter<T> extends AbstractStateRouter<T> implements ConfigurationListener {
    ...
    public BitList<Invoker<T>> doRoute(
        ...
        logger.info("[TAG-ROUTER] TagStateRouter.doRoute called with {} invokers [Thread: {}]", invokers.size(), Thread.currentThread().getName());
        ...
        for (int i = 0; i < invokers.size(); i++) {
            ...
            logger.info("[TAG-ROUTER] Input invoker[{}]: {} (tag: {}, clusterID: {}) [Thread: {}]", i, address, tag, clusterID, Thread.currentThread().getName());
        }
        
        if (CollectionUtils.isEmpty(invokers)) {
            logger.info("[TAG-ROUTER] Invokers is empty, returning empty list [Thread: {}]", Thread.currentThread().getName());
        ...
        logger.info("[TAG-ROUTER] tagRouterRule: {} (valid: {}, enabled: {}) [Thread: {}]", 
            tagRouterRuleCopy, 
        ...
        if (tagRouterRuleCopy == null || !tagRouterRuleCopy.isValid() || !tagRouterRuleCopy.isEnabled()) {
            logger.info("[TAG-ROUTER] Using static tag filtering [Thread: {}]", Thread.currentThread().getName());
            if (needToPrintMessage) {
        ...
        logger.info("[TAG-ROUTER] Requested tag: {} [Thread: {}]", tag, Thread.currentThread().getName());

        // if we are requesting for a Provider with a specific tag
        if (StringUtils.isNotEmpty(tag)) {
            logger.info("[TAG-ROUTER] Processing tag request: {} [Thread: {}]", tag, Thread.currentThread().getName());
            ...
            if (addresses != null) { // null means tag not set
                logger.info("[TAG-ROUTER] Found addresses for tag {}: {} [Thread: {}]", tag, addresses, Thread.currentThread().getName());
                result = filterInvoker(invokers, invoker -> addressMatches(invoker.getUrl(), addresses));
                // if result is not null OR it's null but force=true, return result directly
                if (CollectionUtils.isNotEmpty(result) || tagRouterRuleCopy.isForce()) {
                    logger.info("[TAG-ROUTER] Returning {} invokers for tag {} [Thread: {}]", result.size(), tag, Thread.currentThread().getName());
                    if (needToPrintMessage) {
                        messageHolder.set(
                ...
                logger.info("[TAG-ROUTER] No addresses found for tag {}, checking static tag [Thread: {}]", tag, Thread.currentThread().getName());
            ...
            if (CollectionUtils.isNotEmpty(result) || isForceUseTag(invocation)) {
                logger.info("[TAG-ROUTER] Returning {} invokers for tag {} (force: {}) [Thread: {}]", result.size(), tag, isForceUseTag(invocation), Thread.currentThread().getName());
                if (needToPrintMessage) {
            ...
            // FAILOVER: return all Providers without any tags.
                ...
                logger.info("[TAG-ROUTER] No tagged providers found, returning providers without tags [Thread: {}]", Thread.currentThread().getName());
                ...
                logger.info("[TAG-ROUTER] FAILOVER: returning {} invokers without tags [Thread: {}]", finalResult.size(), Thread.currentThread().getName());
                return finalResult;
            }
        } else {
            logger.info("[TAG-ROUTER] No tag requested, processing all addresses [Thread: {}]", Thread.currentThread().getName());
            // List<String> addresses = tagRouterRule.filter(providerApp);
            // return all addresses in dynamic tag group.
            Set<String> addresses = tagRouterRuleCopy.getAddresses();
            if (CollectionUtils.isNotEmpty(addresses)) {
                logger.info("[TAG-ROUTER] Found {} addresses in dynamic tag group [Thread: {}]", addresses.size(), Thread.currentThread().getName());
                result = filterInvoker(invokers, invoker -> addressNotMatches(invoker.getUrl(), addresses));
                // 1. all addresses are in dynamic tag group, return empty list.
                if (CollectionUtils.isEmpty(result)) {
                    logger.info("[TAG-ROUTER] All addresses are in dynamic tag group, returning empty list [Thread: {}]", Thread.currentThread().getName());
                    if (needToPrintMessage) {
            ...
            logger.info("[TAG-ROUTER] Filtering using static tag group [Thread: {}]", Thread.currentThread().getName());
            if (needToPrintMessage) {
            ...
            BitList<Invoker<T>> finalResult = filterInvoker(result, invoker -> {
                ...
                logger.info("[TAG-ROUTER] Checking invoker {}: tag='{}', isEmpty={} [Thread: {}]", 
                    invoker.getUrl().getAddress(), localTag, isEmpty, Thread.currentThread().getName());
                return isEmpty;
            });
            logger.info("[TAG-ROUTER] Static tag filtering result: {} invokers [Thread: {}]", finalResult.size(), Thread.currentThread().getName());
            return finalResult;
        }
    }
    ...
    private <T> BitList<Invoker<T>> filterUsingStaticTag(BitList<Invoker<T>> invokers, URL url, Invocation invocation) {
        ...
        if (!StringUtils.isEmpty(tag)) {
            ...
            if (CollectionUtils.isEmpty(result) && !isForceUseTag(invocation)) {
                logger.info("[TAG-ROUTER-STATIC] No invokers with tag found, and not force use. Fallback to invokers with empty tag.");
                result = filterInvoker(
                        invokers,
                        invoker -> {
                            String invokerTag = invoker.getUrl().getParameter(TAG_KEY);
                            boolean isEmpty = StringUtils.isEmpty(invokerTag);
                            logger.info("[TAG-ROUTER-STATIC-FALLBACK] Checking invoker " + invoker.getUrl().getAddress() + ", tag='" + invokerTag + "', isEmpty=" + isEmpty);
                            return isEmpty;
                        });
            ...
            result = filterInvoker(
                    invokers, invoker -> {
                        String invokerTag = invoker.getUrl().getParameter(TAG_KEY);
                        boolean isEmpty = StringUtils.isEmpty(invokerTag);
                        logger.info("[TAG-ROUTER-STATIC-NO_TAG] Checking invoker " + invoker.getUrl().getAddress() + ", tag='" + invokerTag + "', isEmpty=" + isEmpty);
                        return isEmpty;
                    });
