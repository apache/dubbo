...
public class CircuitBreaker {
    private static final ErrorTypeAwareLogger logger = LoggerFactory.getErrorTypeAwareLogger(CircuitBreaker.class);
    ...
    public static CircuitBreaker parseFromMap(Map<String, Object> map, String version) {
        CircuitBreaker circuitBreaker = new CircuitBreaker();
        circuitBreaker.setName((String) map.get("name"));

        if (version != null && version.startsWith(RULE_VERSION_V30)) {
            if (map.get("match") != null) {
                circuitBreaker.setMatch(((List<Map<String, Object>>) map.get("match")).stream().map((objectMap) -> {
                    try {
