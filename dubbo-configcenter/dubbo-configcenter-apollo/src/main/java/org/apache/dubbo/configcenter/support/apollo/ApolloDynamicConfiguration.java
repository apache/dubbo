...
public class ApolloDynamicConfiguration implements DynamicConfiguration {
    ...
    ApolloDynamicConfiguration(URL url, ApplicationModel applicationModel) {
        ...
        if (StringUtils.isEmpty(SystemPropertyConfigUtils.getSystemProperty(APOLLO_ADDR_KEY)) && !ANYHOST_VALUE.equals(url.getHost())) {
            SystemPropertyConfigUtils.setSystemProperty(APOLLO_ADDR_KEY, configAddr);
        }
        if (StringUtils.isEmpty(SystemPropertyConfigUtils.getSystemProperty(APOLLO_CLUSTER_KEY)) && configCluster != null) {
