...
public class ReferenceConfig<T> extends AbstractReferenceConfig {
	...
	private final Object finalizerGuardian = new Object() {
		...
		protected void finalize() throws Throwable {
			...
			if (!ReferenceConfig.this.destroyed) {
				...
				/*
				 * 先不做Destroy操作 try { ReferenceConfig.this.destroy(); } catch
				 * (Throwable t) { logger.warn(
				 * "Unexpected err when destroy invoker of ReferenceConfig(" +
				 * url + ") in finalize method!", t); }
				 */
			}
		}
	};
	...
	private void init() {
		...
		if (resolve != null && resolve.length() > 0) {
			...
			if (logger.isWarnEnabled()) {
				if (resolveFile != null && resolveFile.length() > 0) {
					logger.warn("Using default dubbo resolve file " + resolveFile + " replace " + interfaceName + ""
	...
	private static void checkAndConvertImplicitConfig(MethodConfig method, Map<String, String> map,
			Map<Object, Object> attributes) {
		// check config conflict
		if (Boolean.FALSE.equals(method.isReturn()) && (method.getOnreturn() != null || method.getOnthrow() != null)) {
			throw new IllegalStateException(
					"method config error : return attribute must be set true when onreturn or onthrow has been setted.");
		}
		...
		if (onReturnMethod != null && onReturnMethod instanceof String) {
			attributes.put(onReturnMethodKey,
					getMethodByName(method.getOnreturn().getClass(), onReturnMethod.toString()));
		}
		...
		if (onInvokeMethod != null && onInvokeMethod instanceof String) {
			attributes.put(onInvokeMethodKey,
					getMethodByName(method.getOninvoke().getClass(), onInvokeMethod.toString()));
		}
	}
	...
	private T createProxy(Map<String, String> map) {
			...
			if (url != null && url.length() > 0) { // 用户指定URL，指定的URL可能是对点对直连地址，也可能是注册中心URL
				...
				if (urls == null || urls.size() == 0) {
					throw new IllegalStateException("No such any registry to reference " + interfaceName
							+ " on the consumer " + NetUtils.getLocalHost() + " use dubbo version "
							+ Version.getVersion()
		...
		if (c && !invoker.isAvailable()) {
			throw new IllegalStateException("Failed to check the status of the service " + interfaceName
					...
					+ (version == null ? "" : ":" + version) + " from the url " + invoker.getUrl()
