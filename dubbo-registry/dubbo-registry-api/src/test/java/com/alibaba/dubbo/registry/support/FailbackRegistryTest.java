...
public class FailbackRegistryTest {
	...
	public void setUp() throws Exception {
		...
		registryUrl = URL.valueOf("http://1.2.3.4:9090/registry?check=false&file=N/A").addParameter(
				Constants.REGISTRY_RETRY_PERIOD_KEY, String.valueOf(FAILED_PERIOD));
	}
	...
	public void testDoRetry() throws Exception {
		...
		final CountDownLatch latch = new CountDownLatch(3);// 全部共调用3次。成功才会减1.
															// subscribe
															// register的失败尝试不会在做了
		...
		registry.subscribe(
				serviceUrl.setProtocol(Constants.CONSUMER_PROTOCOL).addParameters(
						CollectionUtils.toStringMap("check", "false")), listner);
	...
	public void testDoRetry_subscribe() throws Exception {
		...
		final CountDownLatch latch = new CountDownLatch(1);// 全部共调用4次。成功才会减1.
															// subscribe的失败尝试不会在做了
