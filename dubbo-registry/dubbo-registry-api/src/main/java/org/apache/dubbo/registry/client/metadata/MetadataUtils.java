...
import static org.apache.dubbo.common.constants.CommonConstants.CONNECTIONS;
...
public class MetadataUtils {
    ...
    public static MetadataService referProxy(ServiceInstance instance) {
        ...
        ReferenceConfigBase<MetadataService> referenceConfigBase = buildMetadataServiceReferenceConfig(url);
        referenceConfigBase.setScopeModel(moduleModel);

        ServiceMetadata serviceMetadata = referenceConfigBase.getServiceMetadata();

        serviceMetadata.setServiceType(MetadataService.class);
        serviceMetadata.setServiceKey(URL.buildKey(MetadataService.class.getName(), url.getGroup(), url.getVersion()));

        ConsumerModel consumerModel = new ConsumerModel(MetadataService.class.getName(), null, serviceDescriptor,
            referenceConfigBase, moduleModel, serviceMetadata, new HashMap<>(0));
        ...
        serviceMetadata.setTarget(metadataService);
        serviceMetadata.addAttribute(PROXY_CLASS_REF, metadataService);
    ...
    private static ReferenceConfigBase<MetadataService> buildMetadataServiceReferenceConfig(URL url) {
        ReferenceConfigBase<MetadataService> referenceConfigBase = new ReferenceConfigBase<MetadataService>() {
            @Override
            public MetadataService get() {
                return null;
            }
        };
        referenceConfigBase.setProtocol(url.getProtocol());
        referenceConfigBase.setUrl(url.toFullString());
        referenceConfigBase.setGroup(url.getGroup());
        referenceConfigBase.setVersion(url.getVersion());
        referenceConfigBase.setInterface(MetadataService.class);
        referenceConfigBase.setRetries(url.getParameter(RETRIES_KEY, 0));
        referenceConfigBase.setConnections(url.getParameter(CONNECTIONS, 1));
        referenceConfigBase.setParameters(url.getParameters());
