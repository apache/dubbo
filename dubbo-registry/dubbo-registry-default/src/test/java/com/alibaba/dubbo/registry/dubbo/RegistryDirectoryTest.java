...
public class RegistryDirectoryTest {
	...
	public void test_Constructor_CheckStatus() throws Exception {
		URL url = URL.valueOf("notsupported://10.20.30.40/" + service + "?a=b").addParameterAndEncoded(
				Constants.REFER_KEY, "foo=bar");
	...
	private void testforbid(RegistryDirectory registryDirectory) {
		...
		List<URL> serviceUrls = new ArrayList<URL>();
		serviceUrls.add(new URL(Constants.EMPTY_PROTOCOL, Constants.ANYHOST_VALUE, 0, service, Constants.CATEGORY_KEY,
	...
	private void test_Notified1invokers(RegistryDirectory registryDirectory) {
		...
		List<URL> serviceUrls = new ArrayList<URL>();
		serviceUrls.add(SERVICEURL.addParameter("methods", "getXXX1"));// .addParameter("refer.autodestroy",
																		// "true")
		registryDirectory.notify(serviceUrls);
		Assert.assertEquals(true, registryDirectory.isAvailable());
	...
	public void testParametersMerge() {
		...
		RegistryDirectory<RegistryDirectoryTest> registryDirectory2 = new RegistryDirectory(
				RegistryDirectoryTest.class, regurl);
	...
	/**
	 * When destroying, RegistryDirectory should: 1. be disconnected from
	 * Registry 2. destroy all invokers
	 */
	@Test
	...
	public void testDestroy_WithDestroyRegistry() {
		...
		registryDirectory.subscribe(URL.valueOf("consumer://" + NetUtils.getLocalHost()
				+ "/DemoService?category=providers"));
		registryDirectory.destroy();
	...
	public void testDubbo1UrlWithGenericInvocation() {
		...
		Assert.assertEquals(1, invokers.size());
		Assert.assertEquals(
				serviceURL.setPath(service).addParameters("check", "false", "interface", DemoService.class.getName()),
	 ...
	 * replace the former one. 2. notify with no router url, do nothing to
	 * current routers 3. notify with only one router url, with router=clean,
	 * clear all current routers
	 */
	@Test
	...
	public void testNotifyoverrideUrls_beforeInvoker() {
		...
		Assert.assertEquals("override rute must be first priority", "1",
				invokers.get(0).getUrl().getParameter("timeout"));
		Assert.assertEquals("override rute must be first priority", "5",
	...
	public void testNofityOverrideUrls_disabled_specifiedProvider() {
		...
		durls = new ArrayList<URL>();
		durls.add(URL.valueOf("empty://0.0.0.0?" + Constants.DISABLED_KEY + "=true&" + Constants.CATEGORY_KEY + "="
	...
	public void testNotifyRouterUrls_Clean() {
		...
		URL routerurl = URL.valueOf(Constants.ROUTE_PROTOCOL + "://127.0.0.1:9096/")
				...
				.addParameter(Constants.RULE_KEY, "function test1(){}").addParameter(Constants.ROUTER_KEY, "script"); // FIX
																														// BAD
	...
	public void test_Notified_acceptProtocol1() {
		...
		errorPathUrl = errorPathUrl.addParameterAndEncoded(Constants.REFER_KEY, "interface=" + service
				+ "&protocol=dubbo");
		RegistryDirectory registryDirectory = getRegistryDirectory(errorPathUrl);
		List<URL> serviceUrls = new ArrayList<URL>();
		URL dubbo1URL = URL.valueOf("dubbo://127.0.0.1:9098?lazy=true&methods=getXXX");
