...
public class ExplicitCallbackTest {
	...
	public void initOrResetUrl(int callbacks, int timeout) throws Exception {
		...
		consumerUrl = serviceURL = URL
				.valueOf("dubbo://127.0.0.1:" + port + "/" + IDemoService.class.getName() + "?group=test"
						+ "&xxx.0.callback=true" + "&xxx2.0.callback=true" + "&unxxx2.0.callback=false" + "&timeout="
						+ timeout + "&retries=0" + "&" + Constants.CALLBACK_INSTANCES_LIMIT_KEY + "=" + callbacks);
		// uncomment is unblock invoking
		// serviceURL =
		// serviceURL.addParameter("yyy."+Constants.ASYNC_KEY,String.valueOf(true));
		// consumerUrl =
		// consumerUrl.addParameter("yyy."+Constants.ASYNC_KEY,String.valueOf(true));
	}
