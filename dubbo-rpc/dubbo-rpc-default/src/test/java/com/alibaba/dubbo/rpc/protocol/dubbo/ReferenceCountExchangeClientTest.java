...
public class ReferenceCountExchangeClientTest {
	...
	public void test_counter_error() {
		...
		client.close();
		// client已经被替换为lazyclient lazy
		// client从referenceclientmap中获取，获取到的是上次的client（已经被调用过一次），所以close状态为false
	...
	private void init(int connections) {
		...
		URL demoUrl = URL.valueOf("dubbo://127.0.0.1:" + port + "/demo?" + Constants.CONNECTIONS_KEY + "="
				+ connections);
		URL helloUrl = URL.valueOf("dubbo://127.0.0.1:" + port + "/hello?" + Constants.CONNECTIONS_KEY + "="
				+ connections);
