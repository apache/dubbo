...
public class WrapperResponseObserver<T> extends ServerCallToObserverAdapter<T> {
    ...
    private final GenericPack pack;

    public WrapperResponseObserver(ServerCall call, CancellationContext cancellationContext, String returnType,
                                   GenericPack genericPack) {
        super(call, cancellationContext);
        this.returnType = returnType;
        this.pack=genericPack;
    }
    ...
    public void onNext(Object data) {
        try {
            data = TripleWrapper.TripleResponseWrapper.newBuilder()
                    .setSerializeType(pack.serializationName)
                    .setType(returnType)
                    .setData(ByteString.copyFrom(pack.pack(data)))
                    .build();
        } catch (IOException e) {
            throw TriRpcStatus.INTERNAL.withDescription("Serialize response failed").withCause(e).asException();
        }
        super.onNext(data);
    }
