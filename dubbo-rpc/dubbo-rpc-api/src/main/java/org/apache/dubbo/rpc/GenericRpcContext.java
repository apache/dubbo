 ...
 */
public class GenericRpcContext<T> {

    private final Map<String, T> attachments = new HashMap<>();
    ...
    public <O> O getRequest(Class<O> clazz) {
        return (request != null && clazz.isAssignableFrom(request.getClass())) ? (O) request : null;
    }
     ...
     */
    @SuppressWarnings("unchecked")
    public <O> O getResponse(Class<O> clazz) {
        return (response != null && clazz.isAssignableFrom(response.getClass())) ? (O) response : null;
    }
     ...
     */
    @SuppressWarnings("unchecked")
    public <O> CompletableFuture<O> getCompletableFuture() {
     ...
     * @return context
     */
    public GenericRpcContext<T> setLocalAddress(String host, int port) {
        if (port < 0) {
            port = 0;
        }
     ...
     * @return context
     */
    public GenericRpcContext<T> setLocalAddress(InetSocketAddress address) {
     ...
     * @return context
     */
    public GenericRpcContext<T> setRemoteAddress(String host, int port) {
        if (port < 0) {
            port = 0;
        }
     ...
     * @return context
     */
    public GenericRpcContext<T> setRemoteAddress(InetSocketAddress address) {
    ...
    public GenericRpcContext<T> setRemoteApplicationName(String remoteApplicationName) {
     ...
     * @return attachment
     */
    public T getAttachment(String key) {
     ...
     * @return context
     */
    public GenericRpcContext<T> setAttachment(String key, T value) {
        if (value == null) {
     ...
     * @return context
     */
    public GenericRpcContext<T> removeAttachment(String key) {
        attachments.remove(key);
        return this;
    }
     ...
     * @return attachments
     */
    public Map<String, T> getAttachments() {
        return attachments;
    }
     ...
     * @return context
     */
    public GenericRpcContext<T> setAttachments(Map<String, T> attachment) {
     ...
     * @return context
     */
    public GenericRpcContext<T> set(String key, Object value) {
        if (value == null) {
            values.remove(key);
        } else {
     ...
     * @return value
     */
    public GenericRpcContext<T> remove(String key) {
        values.remove(key);
        return this;
    }
    ...
    public GenericRpcContext<T> setInvokers(List<Invoker<?>> invokers) {
    ...
    public GenericRpcContext<T> setInvoker(Invoker<?> invoker) {
    ...
    public GenericRpcContext setInvocation(Invocation invocation) {
     ...
     */
    @SuppressWarnings("unchecked")
    public <O> CompletableFuture<O> asyncCall(Callable<O> callable) {
        try {
            try {
                setAttachment(ASYNC_KEY, (T) Boolean.TRUE.toString());
                final O o = callable.call();
                //local invoke will return directly
                if (o != null) {
                    if (o instanceof CompletableFuture) {
                        return (CompletableFuture<O>) o;
                    }
                    return CompletableFuture.completedFuture(o);
                } else {
                    // The service has a normal sync method signature, should get future from GenericRpcContext.
                }
            } catch (Exception e) {
            ...
            return new CompletableFuture<O>() {
                @Override
        ...
        return ((CompletableFuture<O>) this.getFuture());
    }
    ...
    public void asyncCall(Runnable runnable) {
        try {
            setAttachment(RETURN_KEY, (T) Boolean.FALSE.toString());
            runnable.run();
        } catch (Throwable e) {
