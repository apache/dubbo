...
public class RpcUtilsTest {
	...
	/**
	 * 正常场景：url中表示了方法异步调用 验证：1. invocationId是否正常设置,2.幂等测试
	 */
	@Test
	...
	/**
	 * 场景：同步调用，不默认添加acctachment 验证：acctachment中没有添加id属性
	 */
	@Test
	...
	/**
	 * 场景：异步调用，默认添加attachement 验证：当原始acctachment为null时，不能报错.
	 */
	@Test
	...
	/**
	 * 场景：强制设置为不添加 验证：acctachment中没有添加id属性
	 */
	@Test
	public void testAttachInvocationIdIfAsync_forceNotAttache() {
		URL url = URL
				.valueOf("dubbo://localhost/?test.async=true&" + Constants.AUTO_ATTACH_INVOCATIONID_KEY + "=false");
		Invocation inv = new RpcInvocation("test", new Class[] {}, new String[] {});
		RpcUtils.attachInvocationIdIfAsync(url, inv);
		Assert.assertNull(RpcUtils.getInvocationId(inv));
	}
	...
	/**
	 * 场景：强制设置为添加 验证：acctachment中有添加id属性
	 */
	@Test
