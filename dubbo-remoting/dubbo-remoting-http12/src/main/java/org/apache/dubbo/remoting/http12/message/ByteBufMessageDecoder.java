...
public interface ByteBufMessageDecoder extends CodecMediaType {

    Object decode(ByteBuf buffer, Class<?> targetType, Charset charset) throws DecodeException;

    default Object decode(ByteBuf buffer, Type targetType, Charset charset) throws DecodeException {
        if (targetType instanceof Class) {
            return decode(buffer, (Class<?>) targetType, charset);
        }
        if (targetType instanceof ParameterizedType) {
            return decode(buffer, (Class<?>) ((ParameterizedType) targetType).getRawType(), charset);
        }
    ...
    default Object decode(ByteBuf buffer, Type targetType) throws DecodeException {
        return decode(buffer, targetType, UTF_8);
    }
