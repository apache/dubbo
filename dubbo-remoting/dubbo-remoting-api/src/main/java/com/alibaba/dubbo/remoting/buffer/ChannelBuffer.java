 ...
 * href=
 * "http://en.wikipedia.org/wiki/Index_(information_technology)#Array_element_identifier"
 * >zero-based indexing</a>. It means the index of the first byte is always
 * {@code 0} and the index of the last byte is always {@link #capacity()
 * capacity - 1}. For example, to iterate all bytes of a buffer, you can do the
 * following, regardless of its internal implementation:
 * <p/>
 * 
 * <pre>
 ...
 * clears the two pointers. Please also note that the semantic of this operation
 ...
 * you can access it directly via the {@link #array()} method. To determine if a
 * buffer is backed by a byte array, {@link #hasArray()} should be used.
 * <p/>
 * <h4>NIO Buffers</h4>
 * <p/>
...
public interface ChannelBuffer extends Comparable<ChannelBuffer> {
	 ...
	 * Otherwise:
	 * <ul>
	 * <li>a non-dynamic buffer will throw an {@link IndexOutOfBoundsException}.
	 * </li>
	 * <li>a dynamic buffer will expand its capacity so that the number of the
	 * {@link #writableBytes() writable bytes} becomes equal to or greater than
	 * the specified value. The expansion involves the reallocation of the
	 * internal buffer and consequently memory copy.</li>
	 * </ul>
	 *
	 * @param writableBytes
	 ...
	 * the number of the transferred bytes (= {@code length}) while
	 ...
	 * {@link #setBytes(int, ChannelBuffer, int, int)} does not. This method
	 ...
	 * have separate indexes and marks. This method is identical to
	 * {@code buf.toByteBuffer(buf.readerIndex(), buf.readableBytes())}. This
	 * method does not modify {@code readerIndex} or {@code writerIndex} of this
	 * buffer.
	 */
	 ...
	 * separate indexes and marks. This method does not modify
	 * {@code readerIndex} or {@code writerIndex} of this buffer.
	 */
	 ...
	 * by the number of the transferred bytes (= {@code length}) while
