...
public class TelnetCodecTest {
	...
	protected AbstractMockChannel getServerSideChannel(URL url) {
		url = url.addParameter(AbstractMockChannel.LOCAL_ADDRESS, url.getAddress()).addParameter(
				AbstractMockChannel.REMOTE_ADDRESS, "127.0.0.1:12345");
		AbstractMockChannel channel = new AbstractMockChannel(url);
		return channel;
	}

	protected AbstractMockChannel getCliendSideChannel(URL url) {
		url = url.addParameter(AbstractMockChannel.LOCAL_ADDRESS, "127.0.0.1:12345").addParameter(
				AbstractMockChannel.REMOTE_ADDRESS, url.getAddress());
		AbstractMockChannel channel = new AbstractMockChannel(url);
		return channel;
	}
	...
	private void testDecode_assertEquals(AbstractMockChannel channel, Object request, Object expectret,
			Object channelReceive) throws IOException {
		// init channel
		if (channel == null) {
	...
	public void testDecode_WithExitByte() throws IOException {
		...
		exitbytes.put(new byte[] { 1, -1, -12, -1, -3, 6 }, false); // must
																	// equal the
																	// bytes
	...
	public void testDecode_Backspace() throws IOException {
		// 32 8 先加空格在补退格.
		testDecode_assertEquals(new byte[] { '\b' }, Codec2.DecodeResult.NEED_MORE_INPUT, new String(
				new byte[] { 32, 8 }));
	...
	/*
	 * @Test() public void testDecode_History_UP_DOWN_MULTI() throws
	 * IOException{ AbstractMockChannel channel = getServerSideChannel(url);
	 * 
	 * String request1 = "aaa\n"; Object expected1 = request1.replace("\n", "");
	 * //init history testDecode_assertEquals(channel, request1, expected1,
	 * null);
	 * 
	 * String request2 = "bbb\n"; Object expected2 = request2.replace("\n", "");
	 * //init history testDecode_assertEquals(channel, request2, expected2,
	 * null);
	 * 
	 * String request3 = "ccc\n"; Object expected3= request3.replace("\n", "");
	 * //init history testDecode_assertEquals(channel, request3, expected3,
	 * null);
	 * 
	 * byte[] UP = new byte[] {27, 91, 65}; byte[] DOWN = new byte[] {27, 91,
	 * 66}; //history[aaa,bbb,ccc] testDecode_assertEquals(channel, UP,
	 * Codec.NEED_MORE_INPUT, expected3); testDecode_assertEquals(channel, DOWN,
	 * Codec.NEED_MORE_INPUT, expected3); testDecode_assertEquals(channel, UP,
	 * Codec.NEED_MORE_INPUT, expected2); testDecode_assertEquals(channel, UP,
	 * Codec.NEED_MORE_INPUT, expected1); testDecode_assertEquals(channel, UP,
	 * Codec.NEED_MORE_INPUT, expected1); testDecode_assertEquals(channel, UP,
	 * Codec.NEED_MORE_INPUT, expected1); testDecode_assertEquals(channel, DOWN,
	 * Codec.NEED_MORE_INPUT, expected2); testDecode_assertEquals(channel, DOWN,
	 * Codec.NEED_MORE_INPUT, expected3); testDecode_assertEquals(channel, DOWN,
	 * Codec.NEED_MORE_INPUT, expected3); testDecode_assertEquals(channel, DOWN,
	 * Codec.NEED_MORE_INPUT, expected3); testDecode_assertEquals(channel, UP,
	 * Codec.NEED_MORE_INPUT, expected2); }
	 */
