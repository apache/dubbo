...
public class DeprecatedTelnetCodec implements Codec {
	...
	private static final List<?> EXIT = Arrays.asList(new Object[] { new byte[] { 3 } /*
																					 * Windows
																					 * Ctrl
																					 * +
																					 * C
																					 */,
			new byte[] { -1, -12, -1, -3, 6 } /* Linux Ctrl+C */, new byte[] { -1, -19, -1, -3, 6 } /*
																								 * Linux
																								 * Pause
																								 */});
	...
	public void encode(Channel channel, OutputStream output, Object message) throws IOException {
		if (message instanceof String) {
			...
			ObjectOutput objectOutput = CodecSupport.getSerialization(channel.getUrl()).serialize(channel.getUrl(),
					output);
	...
	protected Object decode(Channel channel, InputStream is, int readable, byte[] message) throws IOException {
		...
		if (message[message.length - 1] == '\b') { // Windows backspace echo
			try {
				boolean doublechar = message.length >= 3 && message[message.length - 3] < 0; // double
																								// byte
																								// char
