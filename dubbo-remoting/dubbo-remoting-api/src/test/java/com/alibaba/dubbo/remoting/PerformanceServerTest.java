...
public class PerformanceServerTest extends TestCase {
	...
	private static ExchangeServer statServer() throws Exception {
		...
		// 启动服务器
		ExchangeServer server = Exchangers.bind("exchange://0.0.0.0:" + port + "?transporter=" + transporter
				+ "&serialization=" + serialization + "&threadpool=" + threadpool + "&threads=" + threads
				+ "&iothreads=" + iothreads + "&buffer=" + buffer + "&channel.handler=" + channelHandler,
	...
	private static ExchangeServer statTelnetServer(int port) throws Exception {
		...
		ExchangeServer telnetserver = Exchangers.bind("exchange://0.0.0.0:" + port, new ExchangeHandlerAdapter() {
			public String telnet(Channel channel, String message) throws RemotingException {
				if (message.equals("help")) {
				...
				} else if (message.startsWith("restart ")) { // r times [sleep]
																// r 10 or r 10
																// 100
					...
					return "restart server,times:" + times + " stop alive time: " + alive + ",sleep time: " + sleep
							+ " usage:r times [alive] [sleep] \r\ntelnet>";
				} else {
					return "echo: " + message + "\r\ntelnet> ";
				}
