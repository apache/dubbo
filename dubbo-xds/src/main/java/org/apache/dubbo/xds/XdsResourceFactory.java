...
public class XdsResourceFactory {
    ...
    private final Map<String, LdsUpdateWatcher> ldsWatchers = new HashMap<>();
    private final Map<String, RdsUpdateWatcher> rdsWatchers = new HashMap<>();
    private final Map<String, CdsUpdateNodeDirectory> cdsWatchers = new HashMap<>();
    private final Map<String, EdsUpdateLeafDirectory> edsWatchers = new HashMap<>();
    ...
    public void subscribeApp(String appName, EdsListener listener) {
        edsListeners.compute(appName, (key, listeners) -> {
            if (listeners == null) {
                listeners = new ArrayList<>();
                listeners.add(listener);
                LdsUpdateWatcher ldsUpdateWatcher = new LdsUpdateWatcher(appName);
                ldsWatchers.putIfAbsent(appName, ldsUpdateWatcher);
                pilotExchanger.subscribeXdsResource(appName, XdsListenerResource.getInstance(), ldsUpdateWatcher);
            } else {
                listeners.add(listener);
            }
            return listeners;
