...
public class JavaBeanSerializeUtilTest {
	...
	public void testSerialize_Array() throws Exception {
		...
		for (int i = 0; i < integers.length; i++) {
			if (integers[i] == null) {
				...
				Assert.assertEquals(integers[i],
	...
	public void testBeanSerialize() throws Exception {
		...
		Assert.assertEquals(Bean.class.getName(),
				((JavaBeanDescriptor) descriptor.getProperty("type")).getClassNameProperty());
	...
	public void testDeserializeBean() throws Exception {
		...
		Assert.assertEquals(bean.getCollection().iterator().next().getClass(), deserBean.getCollection().iterator()
				.next().getClass());
		Assert.assertEquals(bean.getAddresses().size(), deserBean.getAddresses().size());
		Assert.assertEquals(bean.getAddresses().entrySet().iterator().next().getKey(), deserBean.getAddresses()
				.entrySet().iterator().next().getKey());
		Assert.assertEquals(bean.getAddresses().entrySet().iterator().next().getValue().getClass(), deserBean
				.getAddresses().entrySet().iterator().next().getValue().getClass());
	}
