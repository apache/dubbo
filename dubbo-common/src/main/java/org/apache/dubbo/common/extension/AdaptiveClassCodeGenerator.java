...
public class AdaptiveClassCodeGenerator {
    ...
    private String generateExtNameAssignment(String[] value, boolean hasInvocation) {
        ...
        String formatedExtName=defaultExtName==null?null:"\""+defaultExtName+"\"";
        String lastExtName = formatedExtName;

        for (int i = value.length - 1; i >= 0; --i) {
            if (!"protocol".equals(value[i])) {
                if (hasInvocation) {
                    getNameCode = formatedExtName==null?String.format("url.getMethodParameter(methodName, \"%s\", (String)%s)", value[i], formatedExtName):
                            String.format("url.getMethodParameter(methodName, \"%s\", %s)", value[i], formatedExtName);
                } else {
                    getNameCode = lastExtName==null?String.format("url.getParameter(\"%s\", (String)%s)", value[i], lastExtName):
                            String.format("url.getParameter(\"%s\", %s)", value[i], lastExtName);
                }
            } else {
                getNameCode =lastExtName==null? String.format("( url.getProtocol() == null ? (String)%s : url.getProtocol() )", lastExtName):
                        String.format("( url.getProtocol() == null ? %s : url.getProtocol() )", lastExtName);
            }
            lastExtName = getNameCode;
        }
