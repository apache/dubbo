...
final class FluentLoggerImpl implements FluentLogger {
    ...
    public void internalWarn() {
        if (message != null) {
            if (args != null && args.length > 0) {
                formatAndWarn(INTERNAL_ERROR, message, args);
            } else {
                delegate.warn(INTERNAL_ERROR, cause, extendedInformation, message);
            }
        } else if (messageSupplier != null) {
            if (delegate.isWarnEnabled()) {
                delegate.warn(INTERNAL_ERROR, cause, extendedInformation, messageSupplier.get());
            }
        } else {
            warnMessageMissing();
        }
        ...
        if (message != null) {
            if (args != null && args.length > 0) {
                if (delegate.isWarnEnabled()) {
                    ...
                    delegate.warn(INTERNAL_ERROR, cause, extendedInformation, tuple.getMessage(), t);
                }
            } else {
                delegate.warn(INTERNAL_ERROR, cause, extendedInformation, message, t);
            }
        } else if (messageSupplier != null && delegate.isWarnEnabled()) {
            delegate.warn(INTERNAL_ERROR, cause, extendedInformation, messageSupplier.get(), t);
        }
    }
    ...
    public void internalError() {
        if (message != null) {
            if (args != null && args.length > 0) {
                formatAndError(INTERNAL_ERROR, message, args);
            } else {
                delegate.error(INTERNAL_ERROR, cause, extendedInformation, message);
            }
        } else if (messageSupplier != null) {
            if (delegate.isErrorEnabled()) {
                delegate.error(INTERNAL_ERROR, cause, extendedInformation, messageSupplier.get());
            }
        } else {
            warnMessageMissing();
        }
    ...
    public void internalError(Throwable t) {
        if (message != null) {
            if (args != null && args.length > 0) {
                if (delegate.isErrorEnabled()) {
                    FormattingTuple tuple = MessageFormatter.arrayFormat(message, formatArgs(args));
                    delegate.error(INTERNAL_ERROR, cause, extendedInformation, tuple.getMessage(), t);
                }
            } else {
                delegate.error(INTERNAL_ERROR, cause, extendedInformation, message, t);
            }
        } else if (messageSupplier != null) {
            if (delegate.isErrorEnabled()) {
                delegate.error(INTERNAL_ERROR, cause, extendedInformation, messageSupplier.get(), t);
            }
        } else {
            warnMessageMissing();
        }
    ...
    @Override
    public void log(Level level, String code, String msg, Object... args) {
        switch (level) {
            case TRACE:
                trace(msg, args);
                break;
    ...
    @Override
    public void log(Level level, String code, String msg, Throwable t) {
        switch (level) {
            case TRACE:
                trace(msg, t);
                break;
