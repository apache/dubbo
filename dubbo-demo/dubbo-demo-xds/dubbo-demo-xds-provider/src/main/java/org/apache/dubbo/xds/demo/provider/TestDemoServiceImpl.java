...
public class TestDemoServiceImpl implements DemoService {
    ...
    public String sayHello(String name) {
        ...
        logger.info(
                "Request #{} - Hello {}, scenario: {}, version: {}, pod: {}",
                requestId,
                name,
                testScenario,
                version,
        ...
        if (testScenario != null) {
            switch (testScenario) {
                case "timeout-2s":
                    return handleTimeoutTest(name, version, podName, 3000);
                case "timeout-5s":
                    return handleTimeoutTest(name, version, podName, 1000);
                case "retry-test":
    ...
    private String handleRetryTest(String name, String version, String podName) {
        ...
        logger.info(
                "Retry test: request={}, attempt={}, global counter={}, version={}",
                requestId,
                attemptId,
                currentRetry,
                version);

        // Simple logic: fail first 4 attempts, succeed on 5th attempt
        // This matches VirtualService attempts: 6 (1 original + 5 retries)
        if (currentRetry % 5 != 0) {
            logger.info("Retry test: simulating failure for global attempt {}, version {}", currentRetry, version);
            throw new RpcException(
                    RpcException.NETWORK_EXCEPTION,
                    "Simulated failure for retry test (global attempt: " + currentRetry + ", version: " + version
                            + ")");
        }

        String response = String.format(
                "hello %s from %s (retry test success after %d global attempts)", name, version, currentRetry);
        if (podName != null) {
            response += " (pod: " + podName + ")";
        }

        logger.info("Retry test SUCCESS for version {} after {} global attempts", version, currentRetry);
        return response;
    }
