...
# 构建Docker镜像
build_docker_image() {
    print_info "构建Consumer Docker镜像..."
    ...
    print_info "构建镜像: $CONSUMER_IMAGE"
    ...
    if [ $? -eq 0 ]; then
        print_success "Docker镜像构建完成: $CONSUMER_IMAGE"
    else
...
deploy_to_kubernetes() {
    print_info "部署Consumer到Kubernetes..."
    ...
    # 部署Consumer
    print_info "部署Consumer..."
    kubectl apply -f consumer-services.yaml
    
    if [ $? -eq 0 ]; then
        print_success "Consumer部署配置已应用"
    else
        print_error "Consumer部署失败"
        exit 1
    fi
...
main() {
    ...
    echo "  ./monitor_weighted_routing.sh  # 监控加权路由"
}
