...
/**
 * Create {@link Process} to start zookeeper.
 */
public class StartZookeeperCommandProcessor extends ZookeeperCommandProcessor {

    private static final Logger logger = LoggerFactory.getLogger(StartZookeeperCommandProcessor.class);

    /**
     * The pattern for checking zookeeper is started or not.
     */
    private static final Pattern SERVER_READY_PATTERN = Pattern.compile(".*STARTED.*");
    ...
    protected Process start(ZookeeperInitializer.ZookeeperContext context, int clientPort) {
        ...
        try {
            ...
            throw new DubboTestException(String.format("Failed to start zookeeper server, client port:%d", clientPort), e);
        }
    }
